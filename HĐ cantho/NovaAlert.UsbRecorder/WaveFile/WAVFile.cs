    ...
    public class WAVFile
    {
        ...
        public void Create(String pFilename, bool pStereo, int pSampleRate, short pBitsPerSample, bool pOverwrite)
        {
            ...
            try
            {
                ...
                // Audio format
                buffer[0] = 6; // Use Alaw encoding // 1;
                if (!BitConverter.IsLittleEndian)
        ...
        void WriteWaveHeader(FileStream fs, bool stereo, int sampleRate, short bitsPerSample)
        {
            var wf = new Microsoft.DirectX.DirectSound.WaveFormat()
            {
                AverageBytesPerSecond = sampleRate,
                BitsPerSample = bitsPerSample,
                BlockAlign = 1,
                Channels = (short)(stereo ? 2 : 1),
                FormatTag = Microsoft.DirectX.DirectSound.WaveFormatTag.Pcm,
            ...
            bw.Write(padding);
            bw.Write(wf.Channels);
            bw.Write(wf.SamplesPerSecond);
            bw.Write(wf.AverageBytesPerSecond);
            bw.Write(shBytesPerSample);
            bw.Write(wf.BitsPerSample);
            bw.Write(Data);
